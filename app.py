...
def guardar_imagen_empresa(sb, empresa_id, tipo, archivo):
    """Guarda imagen (logo/firma/sello) como base64 en empresa_imagenes."""
    if archivo is None:
        return
    contenido = archivo.read()
    if len(contenido) > 2 * 1024 * 1024:  # 2MB max
        st.error(f"La imagen {tipo} excede 2MB. Seleccione una más pequeña.")
        return False
    b64 = base64.b64encode(contenido).decode("utf-8")
    content_type = archivo.type or "image/png"
    data = {
    ...
    try:
        existing = sb.table("empresa_imagenes").select("id").eq(
            "empresa_id", empresa_id
        ).eq("tipo", tipo).execute()
        if existing.data:
            sb.table("empresa_imagenes").update(data).eq("id", existing.data[0]["id"]).execute()
        else:
            sb.table("empresa_imagenes").insert(data).execute()
        return True
    except Exception as e:
        st.error(f"Error guardando imagen {tipo}: {e}")
        return False


def obtener_imagenes_empresa(sb, empresa_id):
    """Retorna dict {tipo: {base64, content_type}} de las imágenes de la empresa."""
    try:
        res = sb.table("empresa_imagenes").select("*").eq("empresa_id", empresa_id).execute()
        return {r["tipo"]: r for r in (res.data or [])}
    except Exception:
        return {}


def eliminar_imagen_empresa(sb, empresa_id, tipo):
    """Elimina una imagen específica de la empresa."""
    try:
        sb.table("empresa_imagenes").delete().eq("empresa_id", empresa_id).eq("tipo", tipo).execute()
        return True
    except Exception:
        return False


def mostrar_ui_imagenes(sb, empresa_id, prefix=""):
    """Muestra UI de subida/gestión de imágenes para una empresa.
    prefix: prefijo para keys de Streamlit (evitar duplicados)."""
    st.divider()
    st.markdown("#### 🖼️ Imágenes del Comprobante")
    st.info("Suba el logo, la firma y el sello húmedo. Formatos: PNG, JPG, JPEG. Máximo 2MB cada uno.")

    imagenes = obtener_imagenes_empresa(sb, empresa_id) if empresa_id else {}

    tipos_img = [
        ("logo", "🏢 Logo de la Empresa", "Logo que aparecerá en el encabezado del comprobante"),
        ("firma", "✍️ Firma del Representante Legal", "Firma escaneada del responsable"),
        ("sello", "🔴 Sello Húmedo", "Sello húmedo de la empresa"),
    ]

    col_imgs = st.columns(3)
    archivos_subidos = {}
    for idx, (tipo, label, help_text) in enumerate(tipos_img):
        with col_imgs[idx]:
            st.markdown(f"**{label}**")
            if tipo in imagenes:
                img_data = imagenes[tipo]
                st.image(
                    base64.b64decode(img_data["imagen_base64"]),
                    caption=img_data.get("nombre_archivo", tipo),
                    width=150,
                )
                if st.button(f"🗑️ Eliminar {tipo}", key=f"{prefix}del_{tipo}"):
                    eliminar_imagen_empresa(sb, empresa_id, tipo)
                    st.success(f"{label} eliminada")
                    st.rerun()
            archivo = st.file_uploader(
                f"Subir {tipo}",
                type=["png", "jpg", "jpeg"],
                key=f"{prefix}upload_{tipo}",
                help=help_text,
            )
            if archivo:
                archivos_subidos[tipo] = archivo
...
def page_comprobantes():
    ...
    # Cargar imágenes de la empresa (logo, firma, sello)
    imgs_empresa = obtener_imagenes_empresa(sb, eid)
        ...
        if rets:
            for r in rets:
                ...
                with st.expander(f"📋 {r['numero_comprobante']} — {prov.get('nombre', '')} — {fmt(r['monto_retenido'])} Bs"):
                    ...
                    pdf_bytes = generar_pdf_iva(emp_data, prov, fac, r, imagenes=imgs_empresa)
        ...
        if rets:
            for r in rets:
                ...
                with st.expander(f"📋 {r['numero_comprobante']} — {prov.get('nombre', '')} — {fmt(r['monto_retenido'])} Bs"):
                    ...
                    pdf_bytes = generar_pdf_islr(emp_data, prov, fac, r, imagenes=imgs_empresa)
                    col_dl, col_del = st.columns([3, 1])
                    with col_dl:
                        st.download_button(
                            "📥 Descargar PDF",
...
#  GENERACIÓN DE PDFs
# ══════════════════════════════════════════════════════════════
def generar_pdf_iva(empresa, proveedor, factura, retencion, imagenes=None):
    ...
    if "logo" in imagenes:
        try:
            logo_bytes = base64.b64decode(imagenes["logo"]["imagen_base64"])
            ext = imagenes["logo"].get("content_type", "image/png").split("/")[-1]
            if ext == "jpeg":
                ext = "jpg"
            ...
            pdf.image(tmp.name, x=10, y=5, w=25, h=0)
        except Exception:
            pass
    ...
    # ── Firmas con imágenes ──
    y += 15
    # Firma del representante legal (izquierda)
    if "firma" in imagenes:
        try:
            firma_bytes = base64.b64decode(imagenes["firma"]["imagen_base64"])
            ext = imagenes["firma"].get("content_type", "image/png").split("/")[-1]
            if ext == "jpeg":
                ext = "jpg"
            tmp = tempfile.NamedTemporaryFile(suffix=f".{ext}", delete=False)
            ...
            tmp.close()
            temp_files.append(tmp.name)
            pdf.image(tmp.name, x=40, y=y, w=40, h=0)
        except Exception:
            pass

    # Sello húmedo (centro-izquierda, sobre la línea de firma)
    if "sello" in imagenes:
        try:
            sello_bytes = base64.b64decode(imagenes["sello"]["imagen_base64"])
            ext = imagenes["sello"].get("content_type", "image/png").split("/")[-1]
            if ext == "jpeg":
                ext = "jpg"
            tmp = tempfile.NamedTemporaryFile(suffix=f".{ext}", delete=False)
            tmp.write(sello_bytes)
            tmp.close()
            temp_files.append(tmp.name)
            pdf.image(tmp.name, x=80, y=y - 5, w=25, h=0)
        except Exception:
            pass
...
def generar_pdf_islr(empresa, proveedor, factura, retencion, imagenes=None):
    ...
    if "logo" in imagenes:
        try:
            ...
            tmp.close()
            temp_files.append(tmp.name)
            pdf.image(tmp.name, x=10, y=7, w=22, h=0)
        except Exception:
            pass
    ...
    pdf.set_font("Helvetica", "B", 12)
    pdf.set_xy(35, 10)
    pdf.cell(105, 6, f"{empresa.get('nombre', '')} - {empresa.get('rif', '')}")
    pdf.set_font("Helvetica", "B", 9)
    pdf.set_xy(150, 10)
    ...
    if "firma" in imagenes:
        try:
            ...
            tmp.close()
            temp_files.append(tmp.name)
            pdf.image(tmp.name, x=25, y=y, w=35, h=0)
        except Exception:
            pass

    # Sello húmedo (centro-izquierda)
    if "sello" in imagenes:
        try:
            sello_bytes = base64.b64decode(imagenes["sello"]["imagen_base64"])
            ext = imagenes["sello"].get("content_type", "image/png").split("/")[-1]
            ...
            tmp.close()
            temp_files.append(tmp.name)
            pdf.image(tmp.name, x=65, y=y - 3, w=22, h=0)
        except Exception:
            pass
...
def page_admin():
    ...
    with tab1:
        ...
        if empresas:
            st.divider()
            st.markdown("### 🖼️ Gestionar Imágenes de Empresa")
            opciones_img = {f"{e['nombre']} ({e['rif']})": e for e in empresas}
            sel_img = st.selectbox("Seleccione empresa para imágenes", list(opciones_img.keys()), key="sel_img_emp")
            if sel_img:
                emp_img = opciones_img[sel_img]
                archivos = mostrar_ui_imagenes(sb, emp_img["id"], prefix="img_")
                if archivos:
                    if st.button("💾 Guardar Imágenes", key="btn_guardar_imgs"):
                        ok = True
                        for tipo, archivo in archivos.items():
                            resultado = guardar_imagen_empresa(sb, emp_img["id"], tipo, archivo)
